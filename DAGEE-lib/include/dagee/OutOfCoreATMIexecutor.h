...
class OutOfCoreATMIexecutor : private ATMIdagExecutor<Exec, AllocFactory> {
 ...
 protected:
  ...
  void assignDeviceDataSlots(PartitionDAGptr partDAG) {
    ...
    typename AllocFactory::template Vec<PartNodePtr> prevUser(N, nullptr); // of a device data slot
  ...
  template <typename V>
  void collectPredHandles(PartitionDAGptr partDAG, PartNodePtr node, V& predHandles) {
  ...
  ATMItaskHandle makeMemCpyTasks(PartDataT1* dst, PartDataT2* src, const ATMItaskHandle& predTask) {
    assert(dst && "dst ptr null");
    assert(src && "src ptr null");
    assert(src != dst && "source and destination of memcpy can't be same");
    assert(src->numBlocks() == dst->numBlocks() && "src and dst must have equal numBlocks");
    ...
    for (auto i = src->begin(), end_i = src->end(); i != end_i; ++i) {
      ...
      ATMItaskHandle th =
          atmi_memcpy_async(cparm, dst->template blkPtr<void>(varName),
  ...
  void makeTasksForPartition(PartitionDAGptr partDAG, PartNodePtr pNode, V1& allSources,
                             V2& allSinks) {
    ...
    auto memCpySink = makeMemCpyTasks(part.deviceData(), part.hostData(), part.srcTask());

    auto dagSink = makeDAGtasks(part.dag(), memCpySink);

    part.sinkTask() = makeMemCpyTasks(part.hostData(), part.deviceData(), dagSink);
 ...
 public:
  ...
  void executePartitionDAG(PartitionDAGptr partDAG) {
    ...
    partDAG->forEachNode(
        [&, this](PartNodePtr p) { makeTasksForPartition(partDAG, p, sources, sinks); });
 ...
 protected:
  ...
  void syncMemCpy(PartDataT1* dst, PartDataT2* src, const CopyFunc& copyFunc) {
    for (auto i = src->begin(), end_i = src->end(); i != end_i; ++i) {
      ...
      copyFunc(dst->template blkPtr<uint8_t>(varName), PartDataT2::template blkPtr<uint8_t>(i),
               PartDataT2::blkSz(i));
    }
  }
