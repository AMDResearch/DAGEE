...
uint16_t kernel_dispatch_setup() { return 3 << HSA_KERNEL_DISPATCH_PACKET_SETUP_DIMENSIONS; }
...
void initDispatchPacket(hsa_kernel_dispatch_packet_t* packet, const KernelInstance& ki,
  ...
  // The application is expected to have finalized a kernel (for example, using the finalization
  // API).
...
class SerialHSAexecutor {
  ...
  void init(void) {
    ...
    hsa_queue_t* queue;
    CHECK_HSA(hsa_queue_create(agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX,
                               UINT32_MAX, &queue));
  ...
  void freeArgs(const KernelInstance& ki) { this->mKernArgTail -= ki.mKernArgs.size(); }
 ...
 public:
  SerialHSAexecutor(void) { init(); }

  ~SerialHSAexecutor(void) { shutdown(); }
  ...
  void dispatch(const KernelInstance& ki) {
    ...
    hsa_kernel_dispatch_packet_t* dispatch_packet =
        getPacketAddr<hsa_kernel_dispatch_packet_t>(index);
    ...
    hsa_signal_value_t value =
        hsa_signal_wait_acquire(dispatch_packet->completion_signal, HSA_SIGNAL_CONDITION_LT, 1,
  ...
  void dispatchKiteDAG(const KernelInstance& top, const KernelInstance& left,
                       const KernelInstance& right, const KernelInstance& bottom) {
    uint64_t index = hsa_queue_load_write_index_relaxed(mQueue);
    ...
    hsa_kernel_dispatch_packet_t* bottomPkt =
        getPacketAddr<hsa_kernel_dispatch_packet_t>(index + 5);
    ...
    hsa_signal_value_t value =
        hsa_signal_wait_acquire(bottomPkt->completion_signal, HSA_SIGNAL_CONDITION_LT, 1,
                                UINT64_MAX, HSA_WAIT_STATE_BLOCKED);
