...
struct AtmiMixedDagExecutor {
  ...
  using DAG = typename MixedTaskDag<AllocFactory, false, true,
                                    typename Execs::TaskInstance...>::WithPredSucc;
  using DAGptr = DAG*;
 ...
 public:
  AtmiMixedDagExecutor(Execs&... executors) : mExecutors(std::make_tuple(&executors...)) {}
...
auto makeMixedDagExecutor(Execs&... executors) -> AtmiMixedDagExecutor<AF, Execs...> {
