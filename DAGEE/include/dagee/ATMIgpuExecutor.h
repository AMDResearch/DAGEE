...
struct GpuKernelRegAtmiPolicy {
 ...
 protected:
  ...
  using KernelInfoMap =
      typename AllocFactory::template HashMap<GenericFuncPtr, KernelInfo>;
  ...
  void init(void) {
    ...
    CHECK_ATMI(atmi_module_register_from_memory(
        blobPtrs.data(), blobSizes.data(), blobPlats.data(), numBlobs));
  }
 ...
 public:
  ...
  KernelInfo registerKernel(FuncPtr funcPtr) {
    ...
    if (kiter != mGpuKernels.cend()) {
      ...
      CHECK_ATMI(atmi_kernel_create(&kernel, numArgs, argSizes, 1,
                                    ATMI_DEVTYPE_GPU, name.c_str()));
...
struct GpuKernelLaunchAtmiPolicy {
  ...
  static TaskInstance makeTask(const dim3& blocks, const dim3& threadsPerBlock,
    ...
    return TaskInstance(blocks, threadsPerBlock, kinfo,
                        std::forward<Args>(args)...);
  }
};

using GpuExecutorAtmi = ExecutorSkeletonAtmi<
    GpuKernelRegAtmiPolicy<>,
    impl::KernelLaunchAtmiPolicy<GpuKernelLaunchAtmiPolicy<>::Async> >;
using SynchronousGpuExecutorAtmi = ExecutorSkeletonAtmi<
    GpuKernelRegAtmiPolicy<>,
