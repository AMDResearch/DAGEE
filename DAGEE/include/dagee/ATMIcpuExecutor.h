...
struct ATMIcpuKernelInstance {
  ...
  ATMIcpuKernelInstance(const dim3& numThreads,
                        const ATMIcpuKernelInfo& cpuKinfo, Args&&... args)
      : mThreads(numThreads),
    ...
    mKernArgs = impl::packKernArgsCompOffset(mKernArgOffsets,
  ...
  VecArgAddr argAddresses(void) const {
    ...
    using V = typename std::remove_cv<typename std::remove_reference<decltype(
        mKernArgs)>::type::value_type>::type;

    // auto fptr = mCpuKernelInfo.mFuncPtr;
    argAddrs.emplace_back(reinterpret_cast<void*>(
        &const_cast<ATMIcpuKernelInfo&>(mCpuKernelInfo).mFuncPtr));
...
struct CpuKernelRegAtmiPolicy {
 protected:
  ...
  using CpuKernelInfoMap =
      typename AllocFactory::template HashMap<GenericFuncPtr, atmi_kernel_t>;
 ...
 public:
  ...
  ATMIcpuKernelInfo registerKernel(FuncPtr funcPtr) {
    ...
    if (kiter != mCpuKernels.cend()) {
      return ATMIcpuKernelInfo{kiter->second, fptr};
    } else {
      ...
      CHECK_ATMI(atmi_kernel_create(&kinfo, numArgs, argSizes, 1,
                                    ATMI_DEVTYPE_CPU,
                                    reinterpret_cast<atmi_generic_fp>(wf)));

      mCpuKernels.insert(kiter, std::make_pair(wfKey, kinfo));

      return ATMIcpuKernelInfo{kinfo, fptr};
    }
  }
};

template <typename AllocFactory = dagee::StdAllocatorFactory<>,
          const int SYNCHRONOUS = ATMI_FALSE>
struct CpuKernelLaunchAtmiPolicy {
  ...
  static ATMIkernelHandle kernelHandle(const TaskInstance& ti) {
    return ti.mCpuKernelInfo.mKern;
  }

  template <typename... Args>
  static TaskInstance makeTask(const dim3& threads, const KernelInfo& kinfo,
                               Args&&... args) {
    return TaskInstance(threads, kinfo, std::forward<Args>(args)...);
  }
};

using CpuExecutorAtmi = ExecutorSkeletonAtmi<
    CpuKernelRegAtmiPolicy<>,
    impl::KernelLaunchAtmiPolicy<CpuKernelLaunchAtmiPolicy<>::Async> >;
using SynchronousCpuExecutorAtmi = ExecutorSkeletonAtmi<
    CpuKernelRegAtmiPolicy<>,
