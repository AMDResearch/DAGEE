...
namespace impl {
  ...
  ATMItaskHandle makeInternalTaskForDag(Exec* execPtr, const typename Exec::TaskInstance& ti, const V& predHandles=V()) {
...
private:
   ...
   */
  template <typename> friend struct ExecutorAccess;
public:
   ...
   * initializer_list by typing: executor.launchTask(executor.makeTask(args), {predA, predB, predC});
   */
  template <typename V = std::initializer_list<ATMItaskHandle> >
...
class InitAtmiBase {
  ...
  void finish() { /*atmi_finalize();*/ } // TODO(amber/ashwin) FIXME: this causes a crash when invoked multiple times by different derived instances
...
public:
  ...
  void waitOnTasks(std::initializer_list<ATMItaskHandle> l) const {
    impl::waitOnTasks(l);
  }

  void waitOnTask(const ATMItaskHandle& t) const {
    impl::waitOnTask(t);
  }
