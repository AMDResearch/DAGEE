...
public:
  /*
  explicit PartHostData(std::initializer_list<DataBlock> ptrList): mDataBlocks(ptrList.begin(), ptrList.end())
  {}
  */
  ...
  const VarName& varName(const_iterator i) const { 
    return i->first;
  }
  ...
  static size_t blkSz(const_iterator i) {
    return i->second.mSize;
  }
  /*
  template <typename S>
...
struct PartDeviceData: public PartHostData<AllocFactory> {
  ...
  // use default constructor for Base because we'll fill in dataBlocks using offsets into  mDeviceDataPtr
  ...
  ~PartDeviceData(void) {
    dagee::deviceFree(mDeviceDataPtr);
  }
...
public:
  ...
  :
    mDAGfillFunc(std::forward<F>(dagFillFunc)), 
    mHostData(hostData) 
  {}
