...
public:
  size_t privateSegmentSize() const {
    return mKernHeader->workitem_private_segment_byte_size;
  }

  size_t groupSegmentSize() const {
    return mKernHeader->workgroup_group_segment_byte_size;
  }

  std::uint64_t codeObjAddr() const {
    return mCodeObj;
  }
...
namespace impl {
  ...
  KernelInfoHSA getKernelInfoHSA(KernelFuncPtrStorage Fptr) {
    ...
    struct KernDescMirror {
      ...
      KernDescMirror(const KernDesc& kernDesc) {
        std::memcpy(this, &kernDesc, sizeof(KernDesc));
      }
...
KernelInstance<KI> makeKernelInstance(const dim3& blocks, const dim3& threadsPerBlock, const KI& kinfo, Args&&... args) {
...
KernelInstance<KernelInfoHSA> makeHSAkernelInstance(const dim3& blocks, const dim3& threadsPerBlock, FuncPtr kernPtr, Args&&... args) {
