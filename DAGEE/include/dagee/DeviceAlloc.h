...
void copyToDevice(T* deviceBuffer, const T* hostBuffer, const size_t numElems) {
  CHECK_HIP(hipMemcpy(deviceBuffer, hostBuffer, numElems * sizeof(T),
                      hipMemcpyHostToDevice));
}

template <typename T>
void copyToHost(T* hostBuffer, const T* deviceBuffer, const size_t numElems) {
  CHECK_HIP(hipMemcpy(hostBuffer, deviceBuffer, numElems * sizeof(T),
                      hipMemcpyDeviceToHost));
}

template <typename T>
void copyHostToHost(T* destHostBuffer, const T* srcHostBuffer,
                    const size_t numElems) {
  CHECK_HIP(hipMemcpy(destHostBuffer, srcHostBuffer, numElems * sizeof(T),
                      hipMemcpyHostToHost));
}
...
class DeviceBufferManager {
 ...
 public:
  ...
  inline T* makeDeviceCopy(const T* hostBuffer, const size_t numElems) {
    ...
    assert(std::find(mDevBufs.cbegin(), mDevBufs.cend(), deviceBuffer) ==
           mDevBufs.cend());
  ...
  inline T* makeSharedCopy(const T* hostBuffer, const size_t numElems) {
    ...
    assert(std::find(mHostBufs.cbegin(), mHostBufs.cend(), hostLockedBuffer) ==
           mHostBufs.cend());
