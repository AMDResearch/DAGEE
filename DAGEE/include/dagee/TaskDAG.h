...
struct PredCountBase {
  ...
  //! a shadow copy of mNumPred used for forEachNode_TopoOrder. It  allows resetting count and reusing the DAG when needed
...
struct NodePush : public PredCountBase<>, public NodeBase<D>,
  ...
  template <typename... Args>
  explicit NodePush(Args&&... args):
    PCbase(), NDbase(std::forward<Args>(args)...), SuccBase() {}

};

template <typename D, typename AllocFactory>
struct NodePull : public NodeBase<D>,
                      public NodePredBase<NodePull<D, AllocFactory>, AllocFactory> {
  ...
  template <typename... Args>
  explicit NodePull(Args&&... args):
    NDbase(std::forward<Args>(args)...), PredBase() {}
...
template <typename D, typename AllocFactory>
struct NodeInOut : public PredCountBase<>, public NodeBase<D>,
  ...
  using AdjListTy = typename SuccBase::AdjListTy; // XXX: Hack to work around the fact that SuccBase and PredBase define AdjListTy separately
...
template <typename D, typename AllocFactory=dagee::StdAllocatorFactory<>, bool STORE_PRED = true, bool STORE_SUCC = false>
struct DAGbase {
...
public:
  ...
  template <bool P = STORE_PRED, bool S = STORE_SUCC, typename std::enable_if<P&&S, int>::type = 0>
...
namespace impl {
  ...
  struct GetTypeIdImpl {
    constexpr static const size_t value = std::is_same<In, Arg0>::value ? Index : GetTypeIdImpl<In, Index + 1, Args...>::value;
  };
  ...
  struct GetTypeId {
    constexpr static const size_t value = GetTypeIdImpl<typename std::remove_cv<In>::type, 0ul, Args...>::value;
  ...
  struct MakeIntSeqHelper {
    using type = typename std::conditional< (Beg < End),
  ...
  struct MultiTypePtr {
    ...
    void applyByTypeImpl(F&& func, IntSeq<size_t, Indices...>) {
      const auto ID = id();
      (void)(int[]) {
      (((ID == Indices) ? func(*ptr<DataTypes>()): void()), 0) ...
      };
    }
...
template <typename AllocFactory=dagee::StdAllocatorFactory<>, bool STORE_PRED = true, bool STORE_SUCC = false, typename... DataTypes>
class MixedTaskDag: public DAGbase<impl::MultiTypePtr<DataTypes...>, AllocFactory, STORE_PRED, STORE_SUCC> {
...
private:
  ...
  void deleteNodeImpl(GenericNodeData& gd, impl::IntSeq<size_t, Indices...>) {
    ...
    (void)(int[]) { 
      (((id == Indices) ? deallocateNodeData(std::get<Indices>(mAllocators), gd.template ptr<DataTypes>()): void(0)),0) ...
    };
  }
...
public:
  ...
  typename std::enable_if<!D>::type destroyDAG(DAGptr d) {
    ...
    assert(std::find(mDAGmap.cbegin(), mDAGmap.cend(), d) != mDAGmap.cend() && "DAG not found in DAGmanager");
