...
namespace impl {
  ...
  void waitOnTasks(std::initializer_list<ATMItaskHandle> l) {
    waitOnTasks(l.begin(), l.end());
  }

  void waitOnTask(const ATMItaskHandle& t) { 
    atmi_task_wait(t);
  }
  ...
  void activateTasks(std::initializer_list<ATMItaskHandle> l) {
    activateTasks(l.begin(), l.end());
  }

  void activateTask(const ATMItaskHandle& t) { 
    atmi_task_activate(t);
  }
  ...
  ATMIcopyParam initCopyParam() {
    return initLparamCparam<ATMIcopyParam>();
  }
  ...
  protected:
    ...
    static ATMItaskHandle makeInternalTask(const TaskInstance& ti, const ATMItaskHandle* predsArr, size_t numPreds) {
    ...
    static ATMItaskHandle launchInternalTask(const TaskInstance& ti, const ATMItaskHandle* predsArr, size_t numPreds) {
  ...
  template <typename... Args>
  std::vector<std::uint8_t> packKernArgsCompOffset(std::vector<std::size_t>& argOffsetVec, Args&&... args) {
    // XXX: Look Ma, no loops

    size_t totBufSize = 0;

    argOffsetVec = {impl::argOffsetInBuff(totBufSize, sizeof(BareTy<Args>), alignof(BareTy<Args>))...};
    ...
    // FIXME: C style cast here needs to be replaced by reinterpret_cast. reinterpret_cast throws an error in some cases
    (void)(void *[]){(void *)(new (argBuf.data() + impl::argOffsetInBuff(totBufSize, sizeof(BareTy<Args>), alignof(BareTy<Args>))) BareTy<Args>(std::forward<Args>(args)))...};
