...
inline void atmiMalloc(MemType memType, T*& retBuffer, const size_t numElems) {
  CHECK_ATMI(atmi_malloc(reinterpret_cast<void**>(&retBuffer),
                         numElems * sizeof(T), impl::initMemPlaceAtmi(memType)));
...
template <typename T>
inline void atmiMemcpy(T* dstBuffer,
                       const T* srcBuffer,
                       const size_t numElems) {
  CHECK_ATMI(atmi_memcpy(reinterpret_cast<void*>(dstBuffer),
                         reinterpret_cast<const void*>(srcBuffer),
                         numElems * sizeof(T)));
}
...
class AllocManagerAtmiImpl {
  ...
  T* makeCopyImpl(const V& vec, MemType memType) {
    ...
    assert(std::find(mBufs.cbegin(), mBufs.cend(), retBuffer) == mBufs.cend() && "duplicate buffer");
