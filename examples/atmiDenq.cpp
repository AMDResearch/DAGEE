...
#include <iostream>
#include <vector>
__device__ extern "C" void atmid_task_launch(atmi_lparm_t *lp, unsigned long kernel_id, void *args_region,
...
__global__ void topKern(uint32_t* A_d, uint32_t *B_d, uint32_t *C_d, uint64_t N) {
  ...
  if(i == 0) {
    ...
    // launch left
    atmid_task_launch(&lparm, K_ID_leftKern, (void *)&args, sizeof(Args));
    // launch right
    atmid_task_launch(&lparm, K_ID_rightKern, (void *)&args, sizeof(Args));
  }
}
...
__global__ void leftKern(uint32_t* A_d, uint32_t* B_d, uint32_t *C_d, uint64_t N) {
  ...
  if(i == 0) {
    if(atomicAdd(&d_flag, 1) == 1)
    {
      ...
      args.third_op = C_d;
      args.N = N;
      atmid_task_launch(&lparm, K_ID_bottomKern, (void *)&args, sizeof(Args));
    }
  }
}
