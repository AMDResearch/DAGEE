...
namespace cpputils {
  ...
  template <typename T>
  cpputils::Matrix<T> genSameValMatrix(size_t rows, size_t cols, std::vector<T>& backingVec, const T& defaultVal=T()) {
    ...
    for (size_t i = 0; i < m1.rows(); ++i) {
      for (size_t j = 0; j < m1.cols(); ++j) {
        ...
        if (err(m1(i,j), m2(i,j))) {
          passed = false;
          std::cerr << "ERROR: error at index (" << i << "," << j << "), m1's value = "
            << m1(i,j) << ", m2's value = " << m2(i,j) << std::endl;
        }
      }
    }
