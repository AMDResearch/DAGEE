...
extern "C" {
  ...
  extern void strsm_(char* side, char* uplo, char* transA, char* diag, int* m, int* n, float* alpha, float* A, int* lda, float* B, int* ldb);
...
template <bool LOWER=true, bool TRANS_A=false, bool TRANS_B=false, bool UNIT_DIAG=false, bool LEFT_SIDE=true>
struct BLAS {
  ...
  static void gemm(int rowsC, int colsC, int k, T alpha, T beta, const T* A, int lda, const T* B, int ldb, T* C, int ldc, const F& blasFunc) {
  ...
  static void gemm(int rowsC, int colsC, int k, float alpha, float beta, const float* A, int lda, const float* B, int ldb, float* C, int ldc) {
    gemm(rowsC, colsC, k, alpha, beta, A, lda, B, ldb, C, ldc, &sgemm_);
  }

  static void gemm(int rowsC, int colsC, int k, double alpha, double beta, const double* A, int lda, const double* B, int ldb, double* C, int ldc) {
  ...
  static void gemm(const M& A, const M& B, M& C, T alpha, T beta) {
    ...
    gemm(C.rows(), C.cols(), k, alpha, beta, A.ptr(), A.ldDimSz(), B.ptr(), B.ldDimSz(), C.ptr(), C.ldDimSz());
  }

  template <typename T, typename F>
  static void trsm(int rowsB, int colsB, T alpha, const T* A, int lda, T* B, int ldb, const F& blasFunc) {
    T* _A = const_cast<T*>(A);
  ...
  static void trsm(int rowsB, int colsB, double alpha, const double* A, int lda, double* B, int ldb) {
  ...
  template <typename T, typename F>
  static void syrk(int rowsC, int colsA, T alpha, T beta, const T* A, int lda, T* C, int ldc, const F& blasFunc) {
    T* _A = const_cast<T*>(A);
    Flags f;
  ...
  static void syrk(int rowsC, int colsA, float alpha, float beta, const float* A, int lda, float* C, int ldc) {
    syrk(rowsC, colsA, alpha, beta, A, lda, C, ldc, &ssyrk_);
  }

  static void syrk(int rowsC, int colsA, double alpha, double beta, const double* A, int lda, double* C, int ldc) {
  ...
  static int potrf(int n, float* A, int lda) {
    return potrf(n, A, lda, &spotrf_);
  }

  static int potrf(int n, double* A, int lda) {
    return potrf(n, A, lda, &dpotrf_);
  }
